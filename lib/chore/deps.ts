...
export function build(extraDefaults: Options) {
	...
	async function bump(opts: Options): Promise<void> {
	...
	// TODO options
	async function importMap(opts: Options) {
		const { roots, merged } = extractOptions(opts)
		await ImportMap.generate(roots, merged)
	}
